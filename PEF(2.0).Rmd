...
ggplot(data = GL, aes(x = Zone, y = Density, color = Zone)) +
  ...
  geom_rect(aes(xmin = 2.55, xmax = 4.45, ymin = 0, ymax = 120), fill = NA, color = "grey", alpha = 0.6) +
  ...
  geom_rect(aes(xmin = 4.55, xmax = 6.45, ymin = 0, ymax = 170), fill = NA, color = "grey", alpha = 0.6) +
  theme_classic()
...
for(i in 1:10) {
  ...
  lm.GL <- gam(count ~ s(chwi, k=4) + s(bathy, k=3) + s(temp,k=4), family = poisson, offset = log(effort), data = train)
...
ggplot(data = newdata2) + 
  geom_point(data = CM.static, aes(x = phyto, y = count, color = Year), alpha = 0.5) +
...
for(i in 1:10) {
  ...
  # train a model for testing
  lm.CM <- gam(count ~ s(dist,k=3) + s(phyto,k=4) + s(temp_sd,k=4) + s(temp,k=4) + s(salt,k=4), family = poisson, offset = log(effort), data = train)
  # make predictions on test data
  test$predicted <- predict(lm.CM, newdata = test, type = "response")
  test <- test %>% mutate(null = mean(count), r.residual = (count - predicted)^2, n.residual = (count - null)^2)
  # check the accuracy
  cor(test$count, test$predicted)
  ESS <- 1 - (sum(test$r.residual)/sum(test$n.residual))
  data <- data.frame(
    trial = c(i),
    cor.coef = c(cor(test$count, test$predicted)),
    r.squared = rsq(test$count, test$predicted),
...
ggplot(data = newdata2) + 
  geom_point(data = HP.static, aes(x = phyto, y = PA, color = Year), alpha = 0.5) +
  geom_ribbon(data = newdata2, aes(x = phyto, y = fit, ymin = LL, ymax = UL), alpha = 0.1) + 
  geom_line(data = newdata2, aes(x = phyto, y = fit)) +
...
# PA
lm.HS01 <- gam(PA ~ s(bathy, k=3), data = HS.static)
lm.HS02 <- gam( PA ~ s(topog, k=3) , data = HS.static)
lm.HS03 <- gam( PA ~ s(dist, k=3) , data = HS.static)
lm.HS04 <- gam( PA ~ s(chwi, k=3) , data = HS.static)
lm.HS05 <- gam( PA ~ s(tcur, k=3) , data = HS.static)
lm.HS06 <- gam( PA ~ s(phyto, k=4) , data = HS.static) # this model is actually significant
lm.HS07 <- gam( PA ~ s(temp, k=4) , data = HS.static)
lm.HS08 <- gam( PA ~ s(temp_sd, k=4) , data = HS.static)
lm.HS09 <- gam( PA ~ s(salt, k=4), data = HS.static)
...
# examining model results
summary(lm.HS03)
rm(lm.HS01, lm.HS02, lm.HS04, lm.HS05, lm.HS06, lm.HS07, lm.HS08, lm.HS09)

#FS2
lm.HS3.1 <- gam(PA ~ s(dist, k=3) + s(bathy,k=3), data = HS.static)
lm.HS3.2 <- gam(PA ~ s(dist, k=3) + s(phyto,k=4), data = HS.static)
lm.HS3.3 <- gam(PA ~ s(dist, k=3) + s(temp,k=4), data = HS.static)
lm.HS3.4 <- gam(PA ~ s(dist, k=3) + s(temp_sd,k=4), data = HS.static)
lm.HS3.5 <- gam(PA ~ s(dist, k=3) + s(salt,k=4), data = HS.static)

# selecting the best model
vec_AIC <- AIC(lm.HS3.1, lm.HS3.2, lm.HS3.3, lm.HS3.4, lm.HS3.5)[,2]
vec_AIC
dAIC <- vec_AIC - min(vec_AIC)
AICw <- exp(-dAIC/2) / sum(exp(-dAIC/2))
AICw
...
# examining model results
summary(lm.HS3.1)
rm(lm.HS3.2, lm.HS3.3, lm.HS3.4, lm.HS3.5)

#FS3
lm.HS3.1.1 <- gam(PA ~ s(dist, k=3) + s(bathy,k=3) + s(phyto,k=3), data = HS.static)
lm.HS3.1.2 <- gam(PA ~ s(dist, k=3) + s(bathy,k=3) + s(temp,k=4), data = HS.static)
lm.HS3.1.3 <- gam(PA ~ s(dist, k=3) + s(bathy,k=3) + s(temp_sd,k=4), data = HS.static)
lm.HS3.1.4 <- gam(PA ~ s(dist, k=3) + s(bathy,k=3) + s(salt,k=4), data = HS.static)

# selecting the best model
vec_AIC <- AIC(lm.HS3.1, lm.HS3.1.1, lm.HS3.1.2, lm.HS3.1.3, lm.HS3.1.4)[,2]
vec_AIC
dAIC <- vec_AIC - min(vec_AIC)
AICw <- exp(-dAIC/2) / sum(exp(-dAIC/2))
AICw
...
summary(lm.HS3.1.1)
summary(lm.HS3.1.3)
rm(lm.HS3.1.2, lm.HS3.1.3, lm.HS3.1.4)

#FS4
lm.HS3.1.1.1 <- gam(PA ~ s(dist, k=3) + s(bathy,k=3) + s(phyto,k=4) + s(temp,k=4), data = HS.static)
lm.HS3.1.1.2 <- gam(PA ~ s(dist, k=3) + s(bathy,k=3) + s(phyto,k=4) + s(temp_sd,k=4), data = HS.static)
lm.HS3.1.1.3 <- gam(PA ~ s(dist, k=3) + s(bathy,k=3) + s(phyto,k=4) + s(salt,k=4), data = HS.static)

# selecting the best model
vec_AIC <- AIC(lm.HS3.1.1, lm.HS3.1.1.1, lm.HS3.1.1.2, lm.HS3.1.1.3)[,2]
vec_AIC
dAIC <- vec_AIC - min(vec_AIC)
AICw <- exp(-dAIC/2) / sum(exp(-dAIC/2))
AICw
...
# examining model results
summary(lm.HS3.1.1.2)
# this iteration was only marginally improved and did not add any significant variables
rm(lm.HS3.1.1.1, lm.HS3.1.1.2, lm.HS3.1.1.3)

summary(lm.HS3.1) # best model

## PLOTTING MODEL RESULTS: HSeal
# DISTANCE FROM SHORE
newdata1 <- with(HS.static, data.frame(effort = mean(effort), dist = seq(-1,2.5, 0.05), bathy = mean(bathy), phyto = mean(phyto))) # choose variable to model
newdata2 <- cbind(newdata1, predict(lm.HS3.1, newdata = newdata1, type = "response", se = TRUE))
newdata2 <- within(newdata2, {
  LL <- fit - (1.96 * se.fit)
  UL <- fit + (1.96 * se.fit)
})
...
ggplot(data = newdata2) + 
  geom_point(data = HS.static, aes(x = dist, y = PA, color = Zone), alpha = 0.5) +
  geom_ribbon(data = newdata2, aes(x = dist, y = fit, ymin = LL, ymax = UL), alpha = 0.1) + 
  geom_line(data = newdata2, aes(x = dist, y = fit)) +
  xlab("Distance from Shore (m)") +
  ylab("Liklihood of Presence") +
  theme_classic() # seals are more common close to shore

# WATER DEPTH
newdata1 <- with(HS.static, data.frame(effort = mean(effort), dist = mean(dist), bathy = seq(-2,1.5, 0.05), phyto = mean(phyto))) # choose variable to model
newdata2 <- cbind(newdata1, predict(lm.HS3.1, newdata = newdata1, type = "response", se = TRUE))
newdata2 <- within(newdata2, {
  LL <- fit - (1.96 * se.fit)
  UL <- fit + (1.96 * se.fit)
...
ggplot(data = newdata2) + 
  geom_point(data = HS.static, aes(x = -bathy, y = PA, color = Zone), alpha = 0.5) +
  geom_ribbon(data = newdata2, aes(x = -bathy, y = fit, ymin = LL, ymax = UL), alpha = 0.1) + 
  geom_line(data = newdata2, aes(x = -bathy, y = fit)) +
  xlab("Water Depth (m)") +
  ylab("Liklihood of Presence") +
...
summary(HS06)
rm(HS01, HS02, HS03, HS04, HS05, HS07, HS08)
```
...
vec_AIC <- AIC(HS06, HS5.1, HS5.2, HS5.3, HS5.4, HS5.5, HS5.6, HS5.7)[,2]
vec_AIC
dAIC <- vec_AIC - min(vec_AIC)
AICw <- exp(-dAIC/2) / sum(exp(-dAIC/2))
AICw
...
```
Zone width is the next best predictor of harbor seal presence
...
HS5.6.2 <- glmer(PA ~ temp + (1 | Year) + (1 | Zone), data= HSeal, family = "binomial")
HS5.6.3 <- glmer(PA ~ temp_sd + (1 | Year) + (1 | Zone), data= HSeal, family = "binomial")
HS5.6.4 <- glmer(PA ~ salt + (1 | Year) + (1 | Zone), data= HSeal, family = "binomial")
HS5.6.5 <- glmer(PA ~ dth + (1 | Year) + (1 | Zone), data= HSeal, family = "binomial")
...
# examining model results
summary(HS5.6.1)
rm(HS5.6.2, HS5.6.3, HS5.6.4, HS5.6.5, HS5.6.6)
```
Phytoplankton is the next best predictor 

# HSeal: Forward Selection 4
```{r Logit HSeal: Forward Selection 4}
HS6.5.1.1 <- glmer(PA ~ phyto + dth + (1 | Year) + (1 | Zone), data= HSeal, family = "binomial")
HS6.5.1.2 <- glmer(PA ~ phyto + (1 | Year) + (1 | Zone) + (1 | cruise.gen), data= HSeal, family = "binomial")

# calculating AIC weights
vec_AIC <- AIC(HS5.6.1, HS6.5.1.1, HS6.5.1.2)[,2]
vec_AIC
dAIC <- vec_AIC - min(vec_AIC)
AICw <- exp(-dAIC/2) / sum(exp(-dAIC/2))
AICw
...
rm(HS6.5.1.1)
```
cruise number was the next best predictor. 

# HSeal: Forward Selection 5
```{r Logit HSeal: Forward Selection 5}
HS6.5.1.2.1 <- glmer(PA ~ phyto + dth + (1 | Year) + (1 | Zone) + (1 | cruise.gen), data= HSeal, family = "binomial")

# calculating AIC weights
vec_AIC <- AIC(HS6.5.1.2, HS6.5.1.2.1)[,2]
vec_AIC
dAIC <- vec_AIC - min(vec_AIC)
AICw <- exp(-dAIC/2) / sum(exp(-dAIC/2))
AICw
# this variable did not improve the model
...
1-logLik(HS6.5.1.2)/logLik(nullmod) # 'log Lik.' 0.2307126 (df=5)

## odds ratios ##
exp(cbind(OR = coef(HS6.5.1.2)))

## analyzing predicted probabilities of each variable ##
newdata.zone <- with(HSeal, data.frame(phyto = rep(seq(-2, 4, by = 0.05), times = 36), Zone = rep(c(1,2,3,4,5,6), each = 726), Year = rep(c(2019), times= 4356), cruise.gen = rep(rep(c(1,2,3,4,5,6), each= 121),times=6))) # choose variable to model
newdata.2020 <- with(HSeal, data.frame(phyto = rep(seq(-2, 4, by = 0.05), times = 36), Zone = rep(c(1,2,3,4,5,6), each = 726), Year = rep(c(2020), times= 4356), cruise.gen = rep(rep(c(1,2,3,4,5,6), each= 121),times=6))) # choose variable to model
newdata1 <- rbind(newdata.zone, newdata.2020)
rm(newdata.zone, newdata.2020)

newdata1$predicted <- predict(HS6.5.1.2, newdata = newdata1, type = "response")
newdata1$Zone <- factor(newdata1$Zone)
newdata1$Year <- factor(newdata1$Year)

# mean effect across cruise gen
newdata2 <- newdata1 %>% group_by(phyto, Zone, Year) %>% summarize(pred = mean(predicted))
...
```
Tide height change was the best single predictor

```{r Logit HPorp: Forward Selection 2}
HP5.1 <- glm(PA ~ dth + phyto, data= HPorp, family = "binomial")
HP5.2 <- glm(PA ~ dth + temp, data= HPorp, family = "binomial")
HP5.3 <- glm(PA ~ dth + temp_sd, data= HPorp, family = "binomial")
HP5.4 <- glm(PA ~ dth + salt, data= HPorp, family = "binomial")
HP5.5 <- glmer(PA ~ dth + (1 | Zone), data= HPorp, family = "binomial")
HP5.6 <- glmer(PA ~ dth + (1 | Year), data= HPorp, family = "binomial")
HP5.7 <- glmer(PA ~ dth + (1 | cruise.gen), data= HPorp, family = "binomial")

# selecting the best model
vec_AIC <- AIC(HP05, HP5.1, HP5.2, HP5.3, HP5.4, HP5.5, HP5.6, HP5.7)[,2]
vec_AIC
dAIC <- vec_AIC - min(vec_AIC)
AICw <- exp(-dAIC/2) / sum(exp(-dAIC/2))
AICw
...
rm(HP5.1, HP5.2, HP5.3, HP5.4, HP5.5, HP5.6, HP5.7)
```
None of these predictors improved the model substantially
--------
| HP05 |
--------
...
1-logLik(HP05)/logLik(nullmod)

# confidence intervals for model coefficients
confint.default(HP05)

## odds ratios ##
exp(cbind(OR = coef(HP05)))

## analyzing predicted probabilities of each variable ##
newdata1 <- with(HPorp, data.frame(dth = seq(-2, 2.5, 0.05)))
newdata2 <- cbind(newdata1, predict(HP05, newdata = newdata1, type = "link", se = TRUE))
newdata2 <- within(newdata2, {
...
ggplot(newdata2, aes(x = dth, y = PredictedProb)) + 
...
rm(nb.GL1.1.1, nb.GL1.1.3, nb.GL1.1.4, nb.GL1.1.6)
```
∆ Tide Height is the next best predictor

# GL: Forward Selection 4
```{r GAM GL: Forward Selection 4}
nb.GL1.1.1.1 <- gam(Density ~ s(dth,k=4) + s(phyto,k=4) + s(Zone, bs="re") + s(Year,bs = "re"), family = nb, data=GL)
nb.GL1.1.1.2 <- gam(Density ~ s(dth,k=4) + s(temp,k=4) + s(Zone, bs="re") + s(Year,bs = "re"), family = nb, data=GL)
nb.GL1.1.1.3 <- gam(Density ~ s(dth,k=4) + s(temp_sd,k=4) + s(Zone, bs="re") + s(Year,bs = "re"), family = nb, data=GL)
nb.GL1.1.1.4 <- gam(Density ~ s(dth,k=4) + s(salt,k=4) + s(Zone, bs="re") + s(Year,bs = "re"), family = nb, data=GL)
nb.GL1.1.1.5 <- gam(Density ~ s(dth,k=4) + s(Zone, bs="re") + s(Year,bs = "re") + s(cruise.gen,bs="re") , family = nb, data=GL)


# calculate AIC weights
vec_AIC <- AIC(nb.GL1.1.5, nb.GL1.1.1.1, nb.GL1.1.1.2, nb.GL1.1.1.3, nb.GL1.1.1.4, nb.GL1.1.1.5)[,2]
vec_AIC
dAIC <- vec_AIC - min(vec_AIC)
AICw <- exp(-dAIC/2) / sum(exp(-dAIC/2))
AICw
...
```
temperature standard deviation is the next best predictor but it's not statistically significant or close

# FS 5: 
```{r GAM GL: Forward Selection 5}
nb.GL1.1.1.1.1 <- gam(Density ~ s(dth,k=4) + s(temp_sd,k=4) + s(salt,k=4) + s(Zone, bs="re") + s(Year,bs = "re"), family = nb, data=GL)
nb.GL1.1.1.1.2 <- gam(Density ~ s(dth,k=4) + s(temp_sd,k=4) + s(Zone, bs="re") + s(Year,bs = "re") + s(cruise.gen,bs="re"), family = nb, data=GL)

# calculate AIC weights
vec_AIC <- AIC(nb.GL1.1.1.3, nb.GL1.1.1.1.1, nb.GL1.1.1.1.2)[,2]
vec_AIC
dAIC <- vec_AIC - min(vec_AIC)
AICw <- exp(-dAIC/2) / sum(exp(-dAIC/2))
AICw
...
plot(nb.GL0.1)
```
# neither of these made the model substantially better

# FS 6: Examining if interspecies effects are present
```{r GAM GL: Forward Selection 6}
nb.GL0.1 <- gam(Density ~ s(dth,k=4) + s(temp_sd,k=4) + s(CM.d,k=4) + s(Zone, bs="re") + s(Year,bs = "re"), family = nb, data=GL)

# calculate AIC weights
vec_AIC <- AIC(nb.GL1.1.1.3, nb.GL0.1)[,2]
vec_AIC
dAIC <- vec_AIC - min(vec_AIC)
AICw <- exp(-dAIC/2) / sum(exp(-dAIC/2))
AICw
...
summary(nb.GL0.1)
```
# this variable did improve the model but not substantially
